...
class GroqProvider:
    ...
    def generate(self, request: NarrativeRequest) -> NarrativeResponse:
        """Send narrative request to Groq chat completions endpoint."""
        headers = {
        ...
        try:
            ...
            raise AITimeoutError(f"Groq API request timed out after {self.config.timeout_seconds}s") from err
        except httpx.RequestError as err:
            raise AIProviderError(f"Groq API connection error: {err}") from err
        finally:
        ...
        if response.status_code == 401:
            raise AIAuthenticationError("Groq API authentication failed. Verify GROQ_API_KEY.")
        ...
        try:
            ...
            raise AIProviderError(f"Malformed response payload from Groq: {err}") from err
