"""F1 Race Intelligence — SprintResult repository."""
...
from app.f1.parsing.models import ParsedSprintResult


class SprintResultRepository(BaseRepository):
    """Repository managing SprintResult entities."""

    def get_by_race_and_driver(self, race_id: int, driver_id: int) -> Optional[SprintResult]:
        """Look up a sprint result by composite key (race_id, driver_id)."""
    ...
    def upsert(
        self,
        item: ParsedSprintResult,
        race_id: Optional[int] = None,
        driver_cache: Optional[dict[str, int]] = None,
        constructor_cache: Optional[dict[str, int]] = None,
    ) -> tuple[SprintResult, str]:
        """Idempotently insert or update a single SprintResult record."""
    ...
    def upsert_many(self, items: Sequence[ParsedSprintResult]) -> IngestionStats:
        """Idempotently persist a batch of ParsedSprintResult records."""
        stats = IngestionStats(entity_type="sprint_results", records_processed=len(items))
        if not items:
            return stats
